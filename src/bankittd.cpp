...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/bankitt.conf are parsed in qt/bankitt.cpp's main()
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bankitt:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in bankittd anymore. Use the bankitt-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect bankittd signal handlers
    noui_connect();
