 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Bukake (https://www.bukake.org/),
...
bool AppInit(int argc, char* argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/bukake.conf are parsed in qt/bukake.cpp's main()
    ...
    if (mapArgs.count("-?") || mapArgs.count("-h") ||  mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  bukaked [options]                     " + _("Start Bukake Core Daemon") + "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bukake:"))
                fCommandLine = true;

        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in bukaked anymore. Use the bukake-cli utility instead.\n");
            exit(EXIT_FAILURE);
        }
#ifndef WIN32
...
int main(int argc, char* argv[])
{
    ...
    // Connect bukaked signal handlers
    noui_connect();
